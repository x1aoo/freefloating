...
+ Still have some bugs need to debug
    ...
    + [] Check the function again 
        ...
        + [x] Check the desired trajectory in body_pid
            + [x] The QP method has something wrong that are:
                + [x] The roll motion have cost but can not have the motion
                + [x] Test with other motion are equal 0
                + [x] Check the iterative process
                    + [x] find the error in iterative sentence but do not know how to deal with the problem with iter = 1000?
