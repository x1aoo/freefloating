...
//  std::vector<sensor_msgs::JointState> wrench2Thrusters_iterative(const geometry_msgs::Wrench  & cmd, vpColVector state_pre, ros::NodeHandle &nh) const;
    ...
    sensor_msgs::JointState wrench2Thrusters_3rd(const geometry_msgs::Wrench &cmd, vpColVector state_pre, vpColVector state_poi, vpColVector state_vel, ros::NodeHandle &nh, std::vector<double> gains, Eigen::Matrix3d curR) const;
