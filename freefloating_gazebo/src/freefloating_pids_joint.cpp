...
{
  urdf::Model model;
  //load the name on the 'robot_description' server. But why I can not find the server related with robot_description
  model.initParam("robot_description");
  ros::NodeHandle control_node(nh, "controllers");

  bool cascaded_position = true;
  if(control_node.hasParam("config/joints/cascaded_position"))
    control_node.getParam("config/joints/cascaded_position", cascaded_position);//what means of getParam()?

  std::vector<std::string> joint_names;
  std::vector<double> joint_min, joint_max, vel_max;
  for(const auto &joint: model.joints_)//complete join list of model
  {
    ...
    if(control_node.hasParam(name))
    {
      ...
      if(joint_ptr->type != urdf::Joint::FIXED)
      {
        ...
        // set antiwindup to true - why would anyone set it to false?//why we need this?
